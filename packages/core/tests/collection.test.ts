...
describe('addMany', () => {
  ...
  it('is atomic — no entries inserted if one fails validation', async () => {
    const instance = await create({ dimensions: 2 });
    expect(() =>
      addMany(instance, [
        { id: 'a', embedding: [1, 2] },
...
describe('get', () => {
  ...
  it('returns dequantized embedding for INT8', async () => {
  ...
  it('works with bigint ids', async () => {
    const instance = await create({ dimensions: 2 });
  ...
  it('string and bigint ids do not collide', async () => {
    const instance = await create({ dimensions: 2 });
